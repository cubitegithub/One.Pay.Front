...
using One.Site.Models.Certnacionalidad; 
...
namespace One.Site.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class CertnacionalidadController : ControllerBase
    {
        private readonly ILogger<CertnacionalidadController> _logger;
        private readonly IApiConsumo _apiConsumo;
        public CertnacionalidadController(ILogger<CertnacionalidadController> logger, IApiConsumo apiConsumo)
        {
            _logger = logger;
        ...
        [ActionFilterToken]
        [HttpPost("GetAll")]
        public ActionResult<List<CertnacionalidadOutputType>> Post()
        {
             
            string methodUrl = $"Certnacionalidad/GetAll";
            ...
            try
            {
                ...
                var resultObj = JsonSerializer.Deserialize<List<CertnacionalidadOutputType>>(resultTestObj);
 ...
 [ActionFilterToken]
   ...
   public ActionResult<List<CertnacionalidadOutputType>> PostSelectValue()
   {
       string methodUrl = $"Certnacionalidad/GetAll";
       ...
       catch (Exception exc)
       {
        ...
        {
            string methodUrl = $"Certnacionalidad/Delete/" + id;
        ...
        [ActionFilterToken]
        [HttpPost("Create")]
        public ActionResult<string> Create([FromBody] CertnacionalidadCreateIn modelIn)
        {

            string methodUrl = $"Certnacionalidad/Create";
            modelIn.Uidcertnacionalidad = Guid.NewGuid().ToString();
            try
            {
                ObjectResult? objectResult = null;
                string token = this.HttpContext.GetTokenContextSession(this, out objectResult);
        ...
        [ActionFilterToken]
        [HttpPost("GetById/{*id}")]
        public ActionResult<CertnacionalidadOutputType> GetById([FromRoute] string id)
        {
            string methodUrl = $"Certnacionalidad/GetById/" + id;
            ...
            try
            {
                ...
                var resultObj = JsonSerializer.Deserialize<CertnacionalidadOutputType>(resultTestObj);
        ...
        [ActionFilterToken]
        [HttpPut("Modify")]
        public ActionResult<string> Modify([FromBody] CertnacionalidadUpdateIn modelout)
        {
            string methodUrl = $"Certnacionalidad/Modify";
